    ...
    <div style={{ marginTop: 16, border: '1px solid rgba(102, 232, 255, 0.16)', borderRadius: 16, padding: 16, background: 'rgba(3, 10, 24, 0.76)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{ padding: '8px 12px', borderRadius: 999, border: '1px solid rgba(102,232,255,0.2)', background: 'rgba(95, 209, 255, 0.14)', color: '#f3fcff', cursor: 'pointer' }}
        >
          Add Company Star
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        <button type="button" onClick={() => onConnectionsChange([...connections, createConnection()])} style={{ padding: '8px 12px', borderRadius: 999, border: '1px solid rgba(102,232,255,0.2)', background: 'rgba(95, 209, 255, 0.14)', color: '#f3fcff', cursor: 'pointer' }}>Add Route</button>
      </div>
        ...
        {connections.map((connection) => (
          <div key={connection.id} style={{ border: '1px solid rgba(102, 232, 255, 0.14)', borderRadius: 14, padding: 12, background: 'rgba(255,255,255,0.03)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button type="button" onClick={() => onConnectionsChange(removeConnection(connections, connection.id))} style={{ padding: '4px 8px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,88,88,0.12)', color: '#ffd7d7', cursor: 'pointer' }}>Remove</button>
            </div>
          ...
          <div key={entry.id} style={{ border: '1px solid rgba(102, 232, 255, 0.14)', borderRadius: 14, padding: 12, background: 'rgba(255,255,255,0.03)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" onClick={() => onEntriesChange(reorderPortfolioEntry(entries, entry.id, -1))} disabled={index === 0} style={{ padding: '6px 8px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: '#f3fcff', cursor: index === 0 ? 'not-allowed' : 'pointer', opacity: index === 0 ? 0.6 : 1 }}>↑</button>
                <button type="button" onClick={() => onEntriesChange(reorderPortfolioEntry(entries, entry.id, 1))} disabled={index === entries.length - 1} style={{ padding: '6px 8px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: '#f3fcff', cursor: index === entries.length - 1 ? 'not-allowed' : 'pointer', opacity: index === entries.length - 1 ? 0.6 : 1 }}>↓</button>
                <button type="button" onClick={() => onEntriesChange(removePortfolioEntry(entries, entry.id))} style={{ padding: '6px 8px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,88,88,0.12)', color: '#ffd7d7', cursor: 'pointer' }}>Delete</button>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <button type="button" onClick={() => onEntriesChange(addProjectToEntry(entries, entry.id, { name: 'New Moon', summary: 'Add a new achievement or project.' }))} style={{ padding: '6px 10px', borderRadius: 999, border: '1px solid rgba(102,232,255,0.16)', background: 'rgba(95, 209, 255, 0.14)', color: '#f3fcff', cursor: 'pointer' }}>Add Moon</button>
                </div>
                  ...
                  {entry.projects.map((project, projectIndex) => (
                    <div key={`${entry.id}-${projectIndex}`} style={{ padding: 10, borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}>
                        ...
                        <button type="button" onClick={() => onEntriesChange(removeProject(entries, entry.id, projectIndex))} style={{ padding: '4px 8px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,88,88,0.12)', color: '#ffd7d7', cursor: 'pointer' }}>Remove</button>
                      </div>
                      <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
